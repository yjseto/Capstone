...
Original file is located at
    https://colab.research.google.com/drive/1pAynMe0TQ6pAZ7RVpem4TCHKu9v3-iSM
"""
...
#data = pd.get_dummies(data, columns=['first_harmful_event'])
#print(data.columns)
...
# Create the SVM classifier with class weights
svm_clf = SVC(random_state=42, C=0.1, kernel='linear', gamma='scale', class_weight=class_weights)

'''
# define SVM hyperparameter grid to search (including class_weight)
param_grid = {
...
best_svm_clf = SVC(random_state=42, **best_params)
...
# Get feature importances from coef_
importances = abs(svm_clf.coef_[0])  # Take absolute values for importance
...
plt.title('Feature Importance (from coef_)')
plt.bar(importance_df['Feature'][:10], importance_df['Importance'][:10])  # Show top 10
...
result = permutation_importance(svm_clf, X_test, y_test, n_repeats=2, random_state=42, n_jobs=-1)
...
#dot_data = export_graphviz(best_clf, out_file=None, feature_names=features, class_names=['Low Severity', 'Moderate Severity', 'High Severity'], filled=True, rounded=True, special_characters=True)
